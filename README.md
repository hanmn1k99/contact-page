...
- **Dễ Dàng Tùy Chỉnh:** Không cần động vào mã HTML phức tạp. Toàn bộ nội dung (Tên, Tiểu sử, Hình ảnh, Liên kết, QR Code) đều được cấu hình trong 1 file duy nhất `config_v3.js`.
...
Bạn không cần biết lập trình! Chỉ cần mở file **`config_v3.js`** bằng bất kỳ phần mềm chỉnh sửa văn bản nào (như Notepad trên Windows, hoặc sửa trực tiếp trên GitHub).

Trong file `config_v3.js`, bạn sẽ thấy cấu trúc như sau:
...
- Bấm vào icon bạn thích, bạn sẽ thấy cái tên (ví dụ: `logo-instagram`). Copy tên đó dán vào phần `icon: "..."` trong `config_v3.js`.
...
Do đây là mã HTML thuần, bạn chỉ cần **nhấp đúp chuột vào file `index.html`** để trình duyệt (Chrome, Edge, Safari...) mở lên. Mọi thay đổi bạn lưu ở `config_v3.js` sau khi F5 tải lại trang sẽ áp dụng ngay lập tức.
...
2. Tải toàn bộ các file: `index.html`, `style_v3.css`, `script_v3.js`, `config_v3.js` và các hình ảnh QR của bạn vào thư mục vừa tạo.
